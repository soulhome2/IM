# Правила перехода состояний МИ (v4)

Жизненный цикл инцидента в менеджере инцидентов (МИ): состояния, переходы, права, нормативы и автоэскалация.

Версия 4 опирается на [003 States rules IM.md](003%20States%20rules%20IM.md). Одна операция **Передать** сохранена. Главное изменение: **Закрыть без обработки** — закрыть инцидент, не проходя сценарий, только по отдельному праву. Для массовых сбоев (отключение электричества, потеря связи), не для каждого оператора. Что изменилось относительно v3 — в §16. Как устроены кнопки в очереди и что не взято из внешнего макета карточек — в §18.
...
| **Закрытие без обработки** | Закрыть инцидент, не заполняя сценарий. Для массовых сбоев, когда причина уже известна. Только по праву `incident:close:unprocessed` (§6.1) |
...
stateDiagram-v2
    ...
    new --> closed: Закрыть без обработки
    pending_acceptance --> in_progress: Принять
    pending_acceptance --> new: Отклонить
    pending_acceptance --> closed: Закрыть без обработки
    ...
    in_progress --> closed: Закрыть без обработки
    on_hold --> closed: Закрыть без обработки
...
| `incident:close` | Закрыть с записью результата после заполнения обязательных шагов |
| `incident:close:unprocessed` | Закрыть без прохождения сценария: массовый сбой, причина уже известна |
...
| **Закрыть без обработки** | `new`, `pending_acceptance`, `in_progress`, `on_hold` | `closed` | `incident:close:unprocessed` | указана причина из справочника массовых сбоев; для `in_progress` и `on_hold` — я владелец | `owner` = я (если не был); таймеры останавливаются; сценарий не требуется; в результат пишется признак «без обработки» и причина; запись в журнал. Не считается отменой: инцидент закрыт, в отчётности виден как закрытый без сценария |
...
| Ситуация | Кнопки |
|---|---|
| Мой, `in_progress` | **Назад / Далее** по шагам, **Отложить**, **Передать**, **Вернуть в очередь**, **Закрыть** (сценарий заполнен) или **Закрыть без обработки** (по праву, сценарий не нужен), **Отменить** |
| Мой, `on_hold` | **Возобновить**, **Передать**, **Вернуть в очередь**, **Закрыть без обработки** — по праву |
...
Цвета: обычная работа с инцидентом — основной цвет интерфейса; **Передать** — один акцент передачи ответственности; **Закрыть без обработки** — нейтральная кнопка, не красная: это закрытие, не отмена; перехват и отмена — красный.

Кнопки в очереди стоят **снизу плашки** события, в один ряд, и **видны всегда**. Скрывать их до наведения нельзя: очередь должна работать на телефоне, там нет hover. Чекбокс группового выбора на новых тоже всегда виден.
...
**Закрыть** в очереди — это закрытие без обработки, не обычное «Закрыть инцидент» из карточки. Обычное закрытие по-прежнему только из карточки и только после обязательных шагов.
...
| Нет права `incident:close:unprocessed` | Закрыть без обработки; остаётся обычное закрытие по сценарию, если есть `incident:close` |
...
| Инцидент не мой | правка шагов, макросы, обычное закрытие и отмена — только после перехвата. Исключение: **Закрыть без обработки** для `new` и `pending_acceptance` — по праву, без взятия в работу |
| Обязательные шаги набора «закрытие» не заполнены | обычное **Закрыть** (Отменить и **Закрыть без обработки** — можно, каждое по своему праву) |
...
| Вопрос | Правило |
|---|---|
| Как набирать выборку | чекбокс на каждом новом; в шапке очереди — **Однотипные** (все видимые `new` того же типа, до `max_bulk`) и **Снять** (очистить выбор) |
...
| Закрыть выбранные без обработки | если есть `incident:close:unprocessed` и отмечено ≥ 2 новых одного типа — одна форма причины на всю выборку. Группа сценария не создаётся: каждый инцидент закрывается сам, со своей записью в журнале |
...
## 16. Что изменилось относительно v3

Разбор v1 → v2 остаётся в [002 States rules IM.md](002%20States%20rules%20IM.md) §16, v2 → v3 — в [003 States rules IM.md](003%20States%20rules%20IM.md) §16. Здесь только дельта этой версии.
...
| Кнопки очереди справа колонкой | При трёх–четырёх действиях колонка съедает ширину и на телефоне ломает строку | Кнопки **снизу плашки**, в ряд, всегда видны |
| Выборка однотипных только вручную по чекбоксам | Для массового сбоя отметить десять одинаковых событий долго | В шапке: **Однотипные** и **Снять**; закрытие выбранных без обработки — одной формой |
...
Прототип приведён к v4.

| Было (v3) | Стало |
|---|---|
| Две кнопки в очереди справа | ряд снизу плашки; до четырёх действий |
| Нет закрытия без сценария | `closeUnprocessed` + форма причины |
| Чекбоксы только на строке | плюс **Однотипные** / **Снять** в шапке очереди |
| «Далее» после комментария срабатывало со второго нажатия | одно нажатие записывает комментарий и переходит |
...
|---|---|---|
| Кнопки снизу карточки, не справа | **взято** | При 3–4 действиях ряд снизу читается лучше, чем колонка |
...
| При выборе 2+ прятать кнопки карточек и выезжать плавающую панель | **отклонено** | Групповые действия уже в шапке очереди. Прятать кнопки карточки, когда отмечено несколько, ломает «закрыть вот это одно» |
